...
public class GetDesideri extends ActionSupport implements ServletRequestAware {
	...
	public String execute() {
		...
		if (user != null) {
			if (PermissionMap.getInstance().canRead(user.getType(), Permission.sezioni.DESIDERI)) {

				// Desideri desideri = DesideriDAO.getInstance().getDesideriBySocialCardId(socialCardId);
				Desideri desideri = DesideriDAO.getInstance().getDesideriBySocialCardId(socialCardId, timestamp);

				Gson gson = new Gson();

				jsonString = gson.toJson(desideri);
