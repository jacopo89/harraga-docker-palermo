...
public class GetPenale extends ActionSupport implements ServletRequestAware {
	...
	public String execute() {
		...
		if (user != null) {
			if (PermissionMap.getInstance().canRead(user.getType(), Permission.sezioni.PENALE)) {

				Penale penale = PenaleDAO.getInstance().getPenaleBySocialCardId(socialCardId, timestamp);

				Gson gson = new Gson();

				jsonString = gson.toJson(penale);
