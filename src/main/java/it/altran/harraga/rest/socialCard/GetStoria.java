...
public class GetStoria extends ActionSupport implements ServletRequestAware {
	...
	public String execute() {
		...
		if (user != null) {
			if (PermissionMap.getInstance().canRead(user.getType(), Permission.sezioni.STORIA)) {

				Storia storia = StoriaDAO.getInstance().getStoriaBySocialCardId(socialCardId, timestamp);

				Gson gson = new Gson();

				jsonString = gson.toJson(storia);
